...
Select * From supplier inner join lineitem on s_suppkey = l_suppkey   inner join orders on o_orderkey = l_orderkey and ( o_orderstatus = 'F' or o_totalprice < 100000 ) inner join nation on s_nationkey = n_nationkey where l_quantity > 49 and l_receiptdate >= l_commitdate  ;   Select count(*) from part inner join partsupp on p_partkey=ps_partkey and ( p_size > 49 or ps_availqty > 9998 )  inner join lineitem on ps_suppkey=l_suppkey WHERE l_shipmode IN ('MAIL', 'SHIP', 'TRUCK') and l_quantity<>31 and l_quantity <> 35 AND (l_quantity >= 20)  AND l_commitdate <= l_receiptdate AND l_returnflag NOT IN ('N')


"Select s_suppkey as s_suppkey, s_name, s_address, s_nationkey as s_nationkey, s_phone, s_acctbal, s_comment, l_orderkey, l_partkey, l_suppkey, l_linenumber, l_quantity, l_extendedprice, l_discount, l_tax, l_returnflag, l_linestatus, l_shipdate, l_commitdate, l_receiptdate, l_shipinstruct, l_shipmode, l_comment, o_orderkey as o_orderkey, o_custkey, o_orderstatus, o_totalprice, o_orderdate, o_orderpriority, o_clerk, o_shippriority, o_comment, n_nationkey, n_name, n_regionkey, n_comment From supplier Right Outer Join lineitem ON s_suppkey = l_suppkey and (l_returnflag <> 'R' ) and (l_commitdate <= l_receiptdate ) and (l_commitdate <= l_receiptdate ) and (l_receiptdate >= l_commitdate ) and (l_receiptdate >= l_commitdate )  Left Outer Join nation ON s_nationkey = n_nationkey  Left Outer Join orders ON l_orderkey = o_orderkey and (o_orderstatus = 'F' OR o_orderstatus = 'P') Order By l_commitdate asc"
"Select s_suppkey as s_suppkey, s_name, s_address, s_nationkey as s_nationkey, s_phone, s_acctbal, s_comment, l_orderkey, l_partkey, l_suppkey, l_linenumber, l_quantity, l_extendedprice, l_discount, l_tax, l_returnflag, l_linestatus, l_shipdate, l_commitdate, l_receiptdate, l_shipinstruct, l_shipmode, l_comment, o_orderkey as o_orderkey, o_custkey, o_orderstatus, o_totalprice, o_orderdate, o_orderpriority, o_clerk, o_shippriority, o_comment, n_nationkey, n_name, n_regionkey, n_comment From nation Right Outer Join supplier ON n_nationkey = s_nationkey  Right Outer Join lineitem ON s_suppkey = l_suppkey and l_returnflag <> 'R'  and (l_commitdate <= l_receiptdate ) and (l_receiptdate >= l_commitdate )  Left Outer Join orders ON l_orderkey = o_orderkey and (o_orderstatus = 'F' OR o_orderstatus = 'P') Order By l_commitdate asc"